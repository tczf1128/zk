...
from lib import mcpack as mp
import zkconfig
import zkutils

mp.set_default_version(mp.mcpackv2)
...
def get(request, path='/'):
    ...
    if stat != False:
        ...
        for acl in acls:
            ...
            if acl['scheme'] == 'ip':
                try:
                    hostname = socket.gethostbyaddr(acl['id'])[0].replace('.baidu.com', '')
    ...
    if stat != False:
        if len(data) > 0 and data[0] == '\x10':
            try:
                data = '(McPack v2): %s'%(mp.loads(data))
            except:
                data = 'Neither text nor mcpack.'
        node_dict['data'] = data
    else:
