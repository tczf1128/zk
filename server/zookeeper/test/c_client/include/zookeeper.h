...
enum ZOO_ERRORS_EXT {
    ZWAITTIMEOUT = ZOPERATIONTIMEOUT
};

/**
 * \brief gets the data associated with a node synchronously.
 *
 * This function is similar to \ref zoo_tget except it allows one specify
 * a watcher object rather than a boolean watch flag.
 *
 * \param zh the zookeeper handle obtained by a call to \ref zookeeper_init
 * \param path the name of the node. Expressed as a file name with slashes
 ...
 * It'll be set to the actual data length upon return. If the data is NULL, length is -1.
 * \param stat if not NULL, will hold the value of stat for the path on return.
 * \param timeout the time of waiting for the response, should be positive.
 * \return return value of the function call.
 * ZOK operation completed successfully
 * ZNONODE the node does not exist.
 * ZNOAUTH the client does not have permission.
 ...
 * ZINVALIDSTATE - zhandle state is either in ZOO_SESSION_EXPIRED_STATE or ZOO_AUTH_FAILED_STATE
 * ZMARSHALLINGERROR - failed to marshall a request; possibly, out of memory
 * ZWAITTIMEOUT - not receive the response within the given milliseconds
 */
ZOOAPI int zoo_twget(zhandle_t *zh, const char *path,
        watcher_fn watcher, void* watcherCtx,
        char *buffer, int* buffer_len, struct Stat *stat, int timeout);
 ...
 * ZWAITTIMEOUT - not receive the response within the given milliseconds
 */
ZOOAPI int zoo_tget(zhandle_t *zh, const char *path, int watch, char *buffer,
                    int* buffer_len, struct Stat *stat, int timeout);
 ...
 * ZWAITTIMEOUT - not receive the response within the given milliseconds
 */
ZOOAPI int zoo_tset(zhandle_t *zh, const char *path, const char *buffer,
                   int buflen, int version, int timeout);

/**
 * \brief sets the data associated with a node. This function is the same
 * as zoo_tset except that it also provides access to stat information
 * associated with the znode.
 *
 * \param zh the zookeeper handle obtained by a call to \ref zookeeper_init
 * \param path the name of the node. Expressed as a file name with slashes
 ...
 * ZWAITTIMEOUT - not receive the response within the given milliseconds
 */
ZOOAPI int zoo_tset2(zhandle_t *zh, const char *path, const char *buffer,
                   int buflen, int version, struct Stat *stat, int timeout);
 ...
 * ZWAITTIMEOUT - not receive the response within the given milliseconds
 */
ZOOAPI int zoo_tcreate(zhandle_t *zh, const char *path, const char *value,
        int valuelen, const struct ACL_vector *acl, int flags,
        char *path_buffer, int path_buffer_len, int timeout);
 ...
 * ZWAITTIMEOUT - not receive the response within the given milliseconds
 */
ZOOAPI int zoo_tdelete(zhandle_t *zh, const char *path, int version, int timeout);
 ...
 * ZWAITTIMEOUT - not receive the response within the given milliseconds
 */
ZOOAPI int zoo_tget_children(zhandle_t *zh, const char *path, int watch,
                            struct String_vector *strings, int timeout);
 ...
 * ZWAITTIMEOUT - not receive the response within the given milliseconds
 */
ZOOAPI int zoo_twget_children(zhandle_t *zh, const char *path,
        watcher_fn watcher, void* watcherCtx,
        struct String_vector *strings, int timeout);
 ...
 * ZWAITTIMEOUT - not receive the response within the given milliseconds
 */
ZOOAPI int zoo_tget_children2(zhandle_t *zh, const char *path, int watch,
                            struct String_vector *strings, struct Stat *stat, int timeout);
 ...
 * ZWAITTIMEOUT - not receive the response within the given milliseconds
 */
ZOOAPI int zoo_twget_children2(zhandle_t *zh, const char *path,
        watcher_fn watcher, void* watcherCtx,
        struct String_vector *strings, struct Stat *stat, int timeout);
 ...
 * ZWAITTIMEOUT - not receive the response within the given milliseconds
 */
ZOOAPI int zoo_texists(zhandle_t *zh, const char *path, int watch,
        struct Stat *stat, int timeout);

/**
 * \brief checks the existence of a node in zookeeper synchronously.
 *
 * This function is similar to \ref zoo_texists except it allows one specify
 * a watcher object rather than a boolean watch flag.
 *
 * \param zh the zookeeper handle obtained by a call to \ref zookeeper_init
 * \param path the name of the node. Expressed as a file name with slashes
 ...
 * ZWAITTIMEOUT - not receive the response within the given milliseconds
 */
ZOOAPI int zoo_twexists(zhandle_t *zh, const char *path,
        watcher_fn watcher, void* watcherCtx, struct Stat *stat, int timeout);
 ...
 * ZWAITTIMEOUT - not receive the response within the given milliseconds
 */
ZOOAPI int zoo_tget_acl(zhandle_t *zh, const char *path, struct ACL_vector *acl,
                       struct Stat *stat, int timeout);
 ...
 * ZWAITTIMEOUT - not receive the response within the given milliseconds
 */
ZOOAPI int zoo_tset_acl(zhandle_t *zh, const char *path, int version,
                           const struct ACL_vector *acl, int timeout);
//#endif
